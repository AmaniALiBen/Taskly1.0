...
class Gig {
    ...
    // Add features for a package (used on create)
    ...
    // Replace all features for a package (used on update)
    public function updateFeatures($gig_id, $package_type, array $features) {
        ...
        foreach ($features as $feature) {
            $feature = trim($feature);
            if ($feature !== '') {
                $insertStmt->execute([$package_id, $feature]);
            }
        }
    ...
    public function uploadGigImage($gig_id, $file, $is_cover = false) {
        ...
        $stmt = $this->db->prepare(
            "INSERT INTO gig_images (is_cover, gig_id, extension, uploaded_at)
             VALUES (?, ?, ?, NOW(6))"
        );
        ...
        $stmt = $this->db->prepare("
            SELECT
                ...
                (SELECT CONCAT('/Taskly/uploads/gig-images/', gi.id, '.', gi.extension)
                 ...
                 ORDER BY gi.is_cover DESC, gi.id ASC
                 LIMIT 1) as image
            FROM gigs g
    ...
    public function getCompleteGigForEdit($gig_id) {
        ...
        foreach ($packages as $pkg) {
            ...
            // 999 = unlimited, anything else = fixed number
